...
from connectors.chemical_registry import ChemicalRegistry, MolecularComponent, ComponentStability
...
async def molecular_growth_demo():
    ...
    if result.success:
        ...
        print_result("Stability Score", f"{result.stability_report.stability_score:.1f}", "/100")
        print_result("Bond Energy", f"{result.stability_report.total_bond_energy:.1f}", " kJ/mol")
        print_result("Decomposition Risk", f"{result.stability_report.decomposition_risk:.2%}")
