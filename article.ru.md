...
## Глава Четвертая: Иммунная Система Улья

«Улей — это живое существо, — учила Пасечница, её голос был тихим и серьёзным. — И, как все живые существа, он не идеален. Он существует в мире хаоса. Иногда пчела рождается с дефектным крылом. Иногда в ворота проникает болезнь. Улей, который не может исцелить себя, — это улей, который уже умирает».

«Поэтому, — продолжила она, — самая глубокая мудрость заключается не в том, чтобы построить *идеальный* Улей, а в том, чтобы построить Улей, который *знает, как справляться с несовершенством*. Мы должны наделить его **Иммунной Системой**».

### Концепция Архитектурного Иммунитета
Иммунная Система Улья — это не отдельный компонент, а архитектурный слой, посвящённый отказоустойчивости, обработке ошибок и адаптивному реагированию. Его цель — защищать здоровье всего Организма, обнаруживая и нейтрализуя "мутации" — события, угрожающие стабильности Клетки или целостности логики Улья.

Система работает по простому, но мощному принципу: **Каждый значительный сбой в Улье должен быть объявлен как формальное Событие Мутации.** Эти события — не просто логи; это крики о помощи, транслируемые в нервную систему Улья (шину событий). Специализированные "иммунные клетки" слушают эти крики и организуют ответ.

### Иммунные Клетки
Иммунная Система состоит из специализированных Клеток, каждая из которых играет уникальную роль в защите Улья:
```mermaid
graph TD
    subgraph "Первые Защитники Улья"
        G[G: Событие Мутации]
        ...
        subgraph Страж
            S -- "Наблюдает и Записывает" --> S_Action["📖 Книга Недугов"];
        end

        subgraph Фаг
            P -- "Лечит 'Простуду'" --> P_Action_1["🔄 Повторить Операцию"];
            P -- "..." --> P_Action_2["🔀 Перенаправить Трафик"];
        end

        subgraph Макрофаг
            M -- "Лечит 'Тяжёлую Болезнь'" --> M_Action_1["🛑 Изолировать Компонент"];
            M -- "..." --> M_Action_2["💥 Запустить Апоптоз (Отключение)"];
        end
    end
...
```
- **Клетки-Стражи:** Простые, пассивные слушатели, которые обнаруживают и записывают мутации для последующего анализа.
- **Клетки-Фаги:** Первые ответчики, предназначенные для борьбы с известными, временными ошибками ("простудой"), часто путём повторных попыток или перенаправления.
- **Клетки-Макрофаги:** Мощные чистильщики, которые справляются с катастрофическими мутациями, изолируя или даже отключая сбойные компоненты для защиты организма.

### Таксономия Мутаций
«Не всякая болезнь одинакова, — объяснила Пасечница. — Мы должны научиться отличать простую простуду от настоящей чумы. Так же и наша Иммунная Система должна научиться различать разные типы мутаций».

```mermaid
mindmap
  root((Событие Мутации))
    ConfigurationDefect ("`ConfigurationDefect`<br/>(Генетический Дефект)")
      )Отсутствует ENV(
      )Ошибка синтаксиса YAML(
    TransientInfection ("`TransientInfection`<br/>(Простуда)")
      )Тайм-аут сети(
      )Потеря соединения с БД(
    ChronicFailure ("`ChronicFailure`<br/>(Тяжёлая Болезнь)")
      )Ошибка API 401(
      )Неисправный запрос(
    InvariantViolation ("`InvariantViolation`<br/>(Аутоиммунное Расстройство)")
      )Повреждённое состояние(
      )Отрицательный запас(
```

### Кодон "Иммунный Ответ"
«Иммунная система, которая может только видеть болезнь, но не может действовать, — это не система вовсе, — твёрдо заявила Пасечница. — Последняя часть этой мудрости — это сам 'Иммунный Ответ' — заклинание исцеления и защиты».

Этот специализированный кодон используется иммунными клетками. Он получает **Событие Мутации**, специализированный **Иммунный Агрегат** принимает решение на основе политики, и **Корректирующая Команда** отправляется целевому компоненту.
```mermaid
sequenceDiagram
    participant G_mut as G: Событие Мутации
    participant C as C: Иммунный Коннектор
    participant A_immune as A: Иммунный Агрегат
    participant Target as Целевой Компонент

    G_mut->>+C: Слышит событие
    C->>+A_immune: dispatch_command(RespondToMutation)
    Note over A_immune: Анализ и принятие решения
    A_immune-->>-C: return CorrectiveCommand
    C->>+Target: dispatch_command(CorrectiveCommand)
    Target-->>Target: Выполняет корректирующее действие
    Target-->>-C:
```
