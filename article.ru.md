...
«Чтобы по-настоящему понять, — сказала Пасечница, доставая старую, зачарованную карту, — вы должны увидеть улей целиком».
...
## Книга Кодонов Пасечницы

«Четыре примитива ATCG — это буквы нашего алфавита, — продолжила Пасечница, открывая тяжёлую, обтянутую кожей книгу. — Но одних букв недостаточно. Они образуют 'кодоны' — слова с особым, мощным значением. Это фундаментальные заклинания нашего ремесла».

«Давайте изучим три самых главных кодона».

### 1. Кодон "Обработать Команду"
«Это самое распространённое заклинание, слово, означающее 'сделать' или 'изменить', — сказала она. — Оно описывает, как внешний мир может попросить наш улей выполнить действие».

Схема проста: внешний запрос поступает в **Коннектор (C)**. Коннектор преобразует его в команду и вызывает **Агрегат (A)**. Агрегат применяет свои правила, изменяет своё состояние и испускает **Событие Генезиса (G)**, чтобы объявить о случившемся.

```mermaid
graph LR
    subgraph "Кодон: Обработать Команду"
        C[C: Коннектор] --> A[A: Агрегат];
        A --> G[G: Событие Генезиса];
    end
...
### 2. Кодон "Запросить Данные"
«Иногда мир не хочет изменять наш улей, а лишь задать вопрос, — объяснила Пасечница. — Для этого мы используем кодон 'Запросить Данные', слово, означающее 'увидеть' или 'узнать'».

Этот паттерн предназначен для чтения данных. Запрос поступает через **Коннектор (C)**. Он передаётся в **Трансформацию (T)** без состояния, которая собирает необходимую информацию (возможно, из одного или нескольких оптимизированных для чтения хранилищ) и преобразует её в объект передачи данных (DTO) для отправки обратно во внешний мир. Состояние не изменяется; События Генезиса не создаются.

```mermaid
graph LR
    subgraph "Кодон: Запросить Данные"
        C_In[C: Входной Коннектор] --> T[T: Трансформация];
        T --> C_Out[C: Выходной Коннектор];
    end
...
### 3. Кодон "Реагировать на Событие"
«Последнее заклинание — самое волшебное, — прошептала она. — Именно так разные части улья или даже разные ульи на пасеке общаются друг с другом. Это слово, означающее 'слушать' и 'реагировать'».

Этот паттерн начинается со слушающего **Коннектора (C)**, подобного уху пчелы, настроенному на частоту определённого "виляющего танца" (События Генезиса из другого домена). Услышав событие, он преобразует его в команду и, подобно кодону "Обработать Команду", вызывает **Агрегат (A)** в своём собственном домене. Это может вызвать цепную реакцию, в результате которой второй агрегат произведёт собственное **Событие Генезиса (G)**.

```mermaid
graph LR
    subgraph "Кодон: Реагировать на Событие"
        G_In[G: Входящее Событие] --> C[C: Слушающий Коннектор];
        C --> A[A: Агрегат];
        A --> G_Out[G: Исходящее Событие];
    end
...
*Этот раздел отходит от нашей сказки, чтобы представить более технический взгляд на обсуждаемые нами паттерны.*
...
И последняя мысль по этой метафоре: если Протокол Пыльцы определяет *гены* (структуру события), то сырые, сериализованные данные, которые передаются по сети — строка JSON, байты Protobuf — это и есть сама *генетическая последовательность*. Это `tataaaaataaaataaaaaa...` нашей системы, физическое выражение логического гена.
...
Это конечная цель: архитектура, настолько хорошо определённая, что она становится живой фабрикой для своих собственных компонентов. Это слияние Domain-Driven Design, GitOps и Model-Driven Development, создающее систему, которая не просто построена, а по-настоящему *выращена*.

### Доказательство концепции: Генератор Кодонов

Чтобы продемонстрировать мощь идеи "самосоздающихся систем", был создан скрипт-доказательство концепции `codon_generator.py`. Этот скрипт на Python читает простой файл `feature.yml`, который декларативно определяет функцию, используя наши паттерны кодонов.

**Как это работает:**
1.  **Определите Функцию:** Вы описываете новую функцию в `feature.yml`, указывая, какие кодоны она использует.
2.  **Запустите Генератор:** Вы выполняете команду: `python codon_generator.py feature.yml`.
3.  **Получите Шаблонный Код:** Скрипт выводит определения классов Python для необходимых Агрегатов и Коннекторов, создавая готовый скелет, который разработчик может заполнить конкретной бизнес-логикой.

Этот скрипт — простая иллюстрация глубокой концепции: когда архитектура хорошо определена и её паттерны понятны, мы можем автоматизировать создание значительной части нашего кода, освобождая разработчиков для концентрации на том, что действительно важно.
