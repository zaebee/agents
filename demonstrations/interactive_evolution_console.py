...
try:
    from dna_core.royal_jelly.quantum_dna_genetic_programming import (
        ...
        EvolvingComponent, EvolutionMetrics
    )
...
Type 'exit' or 'quit' to close the console.
    ...
    def do_start(self, args):
        ...
        population_size = int(params[0]) if len(params) > 0 else self.session.settings['population_size']
        max_generations = int(params[1]) if len(params) > 1 else self.session.settings['max_generations']
        ...
        self._log_command("start", {"population_size": population_size, "max_generations": max_generations})
    ...
    def do_mutate(self, args):
        ...
        print(f"🧬 Triggering manual mutation...")
        ...
        if success:
            ...
            print(f"❌ Mutation failed - component remained stable")
        
        self._log_command("mutate", {"component_id": component_id, "mutation_type": mutation_type, "success": success})
    ...
    def do_crossover(self, args):
        ...
        self._log_command("crossover", {"parent1": parent1, "parent2": parent2, "child_fitness": child_fitness})
    ...
    def do_set_fitness(self, args):
        ...
        self._log_command("set_fitness", {"function_name": func_name, "expression": expression})
    ...
    def do_inject_code(self, args):
        ...
        self._log_command("inject_code", {"component_name": component_name, "code_length": len(code_snippet)})
    ...
    def do_visualize(self, args):
        ...
        if self.showcase:
            self.visualization_task = asyncio.create_task(
                self.showcase.start_live_showcase(vis_mode, duration_minutes=duration/60)
            )
        else:
    ...
    def do_status(self, args):
        ...
        if self.current_population:
            avg_fitness = sum(comp.get('fitness', 0) for comp in self.current_population) / len(self.current_population)
            print(f"   Average Fitness: {avg_fitness:.3f}")
            best_fitness = max(comp.get('fitness', 0) for comp in self.current_population)
        ...
        print(f"   Custom Fitness Functions: {len(self.session.custom_fitness_functions)}")
    ...
    def do_save_state(self, args):
        ...
        """
        filename = args.strip() if args else f"evolution_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def do_bookmark(self, args):
        ...
        """
        params = args.split(' ', 1) if args else []
        name = params[0] if len(params) > 0 else f"bookmark_{len(self.session.bookmarks)}"
    ...
    def do_scenario(self, args):
        ...
        if self.scenario_simulator:
            asyncio.create_task(
                self.scenario_simulator.run_scenario_simulation(scenario_id, accelerated=True)
            )
        else:
    ...
    async def _run_evolution_loop(self, population_size: int, max_generations: int):
        ...
        try:
            while (self.evolution_state in [EvolutionState.RUNNING, EvolutionState.PAUSED] and 
    ...
    async def _evolution_step(self):
        ...
        for component in self.current_population:
            if random.random() < self.session.settings['mutation_rate']:
                ...
                component['fitness'] = max(0.0, min(1.0, component['fitness'] + fitness_change))
        ...
        event = {
            ...
            'avg_fitness': sum(c['fitness'] for c in self.current_population) / len(self.current_population)
        }
