...
from .aggregate import ${ClassName}Aggregate

class ${ClassName}Connector:
    """
    The connector for the ${ComponentName} component.
    ...
    def process_request(self, data: dict) -> None:
        ...
        print(f"Connector received request for ${ComponentName}: {data}")

        # 1. Create the command
        command = ${ClassName}Command(payload=data)

        # 2. Create a new aggregate instance
        # In a real app, you might fetch an existing aggregate from a repository.
        aggregate_id = str(uuid.uuid4())
        aggregate = ${ClassName}Aggregate(aggregate_id)
        ...
        print(f"${ClassName} process complete. Aggregate ID: {aggregate.id}")
