...
from .transformations.honeyprint_generator import HoneyprintGenerator, create_example_honeyprint
from .transformations.reaction_engine import ReactionEngine
from .aggregates.molecular_analyzer import MolecularAnalyzer
from .connectors.chemical_registry import ChemicalRegistry, SearchQuery, ComponentStability
...
@click.option('--temperature', type=float, default=298.0, help='Reaction temperature (K)')
...
def build(blueprint_name, component_name, temperature, pressure, catalyst, params):
    ...
    async def _build():
        ...
        click.echo(f"👑 Building component '{component_name}' using blueprint '{blueprint_name}'...")
        
        result = await queen.build_component(blueprint_name, component_name, custom_params)
        ...
        if result.success:
            ...
            click.echo(f"   Bond Energy: {result.stability_report.total_bond_energy:.1f} kJ/mol")
...
def health():
    ...
    click.echo(f"Overall Stability Score: {click.style(f'{metrics['stability_score']:.1f}/100', fg='green' if metrics['stability_score'] > 80 else 'yellow')}")
...
@click.option('--adapters', default='REST,gRPC,SQL,Events,Auth,CLI', help='Comma-separated adapter list')
...
def generate(output, core, adapters, width, height):
    ...
    click.echo(f"🐝 Generating honeyprint for {core} with {len(adapter_list)} adapters...")
...
@click.option('--connections', help='Comma-separated connection pairs (from:to,from:to)')
def stability(component_name, formula, connections):
    ...
    report = analyzer.analyze_molecular_stability(component_name, formula, connection_list)
    
    click.echo(f"\n📊 Stability Analysis Results:")
    click.echo(f"   Stability Level: {click.style(report.stability_level.value, fg='green' if report.stability_score > 80 else 'yellow')}")
...
@click.option('--stability', type=click.Choice(['experimental', 'stable', 'deprecated', 'radioactive', 'aromatic']), help='Filter by stability')
...
def search(name, formula, stability, limit):
    ...
    click.echo(f"✅ Found {len(results)} components (showing top {min(limit, len(results))}):\n")
...
def popular():
    ...
    for i, (comp, usage_count) in enumerate(rankings, 1):
        click.echo(f"{i}. {click.style(comp.name, fg='blue', bold=True)} - {usage_count} uses")
...
def lab(port):
    ...
    try:
        with socketserver.TCPServer(("", port), handler) as httpd:
            click.echo(f"🧪 Starting Chemistry Laboratory on port {port}...")
