...
class MolecularComponent:
    ...
    reaction_products: List[str] = field(
        default_factory=list
    )  # What reactions create this
    decomposition_products: List[str] = field(
        default_factory=list
    )  # What this breaks down into
...
class ChemicalRegistry:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_formula ON components (molecular_formula)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_stability ON components (stability)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_elements ON components (element_composition)"
            )
    ...
    def register_component(self, component: MolecularComponent) -> str:
        ...
        component_id = self._generate_component_id(
            component.name, component.molecular_formula
        )
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if existing:
                ...
                print(
                    f"🔬 Created new isotope for existing component: {component.name}"
                )
                return component_id
        ...
        print(
            f"✅ Registered new component: {component.name} ({component.molecular_formula})"
        )
        return component_id
    ...
    def search_components(self, query: SearchQuery) -> List[MolecularComponent]:
        ...
        for row in rows:
            ...
            if query.elements:
                if not all(
                    element in component.element_composition
    ...
    def get_all_components(
        self, limit: int = None, offset: int = 0, order_by: str = "creation_date DESC"
    ) -> List[MolecularComponent]:
        """Get all registered components with optional pagination and ordering"""
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row

            query = f"SELECT * FROM components ORDER BY {order_by}"
            params = []
        ...
        components = [self._row_to_component(row) for row in rows]
        print(f"📊 Retrieved {len(components)} components from registry")
        return components

    def get_component_count(self) -> int:
        """Get total number of registered components"""
        with sqlite3.connect(self.db_path) as conn:
            result = conn.execute("SELECT COUNT(*) FROM components").fetchone()
            return result[0] if result else 0

    def find_similar_components(
        self, component_id: str, similarity_threshold: float = 0.8
    ) -> List[Tuple[MolecularComponent, float]]:
    ...
    def record_usage(
        self, component_id: str, context: str = "", performance_score: float = 1.0
    ):
    ...
    def get_popularity_rankings(
        self, limit: int = 10
    ) -> List[Tuple[MolecularComponent, int]]:
        """Get the most popular components by usage count"""
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                "SELECT * FROM components ORDER BY usage_count DESC LIMIT ?", (limit,)
            ).fetchall()
    ...
    def analyze_composition_trends(self) -> Dict[str, Dict[str, int]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            rows = conn.execute(
                "SELECT element_composition, stability FROM components"
            ).fetchall()
    ...
    def export_registry(self, file_path: str, format: str = "json") -> bool:
        ...
        try:
                ...
                with open(file_path, "w", newline="") as f:
                    if all_components:
                        writer = csv.DictWriter(
                            f, fieldnames=asdict(all_components[0]).keys()
                        )
                        writer.writeheader()
                        for comp in all_components:
            ...
            print(
                f"📁 Registry exported to {file_path} ({len(all_components)} components)"
            )
            return True
    ...
    def _row_to_component(self, row: sqlite3.Row) -> MolecularComponent:
        ...
        return MolecularComponent(
            ...
            similar_components=json.loads(row["similar_components"])
            if row["similar_components"]
            else [],
            ...
            reaction_products=json.loads(row["reaction_products"])
            if row["reaction_products"]
            else [],
            decomposition_products=json.loads(row["decomposition_products"])
            if row["decomposition_products"]
            else [],
        )

    def _calculate_similarity(
        self, comp1: MolecularComponent, comp2: MolecularComponent
    ) -> float:
...
if __name__ == "__main__":
    ...
    # Test search
    search_results = registry.search_components(
        SearchQuery(stability=ComponentStability.STABLE, min_bond_energy=100.0)
    )
