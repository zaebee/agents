...
@hatch.command(name='command', help="Scaffolds a 'Handle Command' pattern (C -> A -> G).")
...
@hatch.command(name='event', help="Scaffolds a 'React to Event' pattern (G -> C -> A -> G).")
@click.argument('component_name')
def hatch_event_cli(component_name):
    cli_connector.hatch('event', component_name)

@hatch.command(name='immune', help="Scaffolds an 'Immune Response' pattern (G -> C -> A -> C).")
@click.argument('component_name')
