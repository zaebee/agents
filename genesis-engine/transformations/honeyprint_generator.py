            ...
            if element in element_counts:
                ...
                if count > 1:
                    formula_parts.append(
                        f"{element}₂" if count == 2 else f"{element}{count}"
                    )
                else:
...
class HoneyprintGenerator:
    ...
    def create_hexagonal_molecule(
        self, core_name: str, adapters: List[str]
    ) -> HoneyprintMolecule:
        ...
        hex_radius = 100
        for i, adapter_name in enumerate(
            adapters[:6]
        ):  # Limit to 6 for perfect hexagon
            ...
            adapter_atom = MolecularAtom(
                element=MolecularElement.CONNECTOR, name=adapter_name, x=x, y=y
            )
            molecule.atoms.append(adapter_atom)

            # Create aromatic bond from core to adapter
            bond = MolecularBond(
                from_atom=core_atom, to_atom=adapter_atom, bond_type=BondType.AROMATIC
            )
    ...
    def add_external_connections(
        self, molecule: HoneyprintMolecule, connections: Dict[str, str]
    ) -> HoneyprintMolecule:
...
<svg width="{self.width}" height="{self.height}" xmlns="http://www.w3.org/2000/svg" font-family="monospace" font-size="12">
    ...
    <!-- Bonds (draw first, so they appear behind atoms) -->'''
        ...
        for atom in molecule.atoms:
            css_class = (
                "external" if atom.color == "#6B8E23" else atom.element.name.lower()
            )
...
def create_example_honeyprint():
    ...
    connections = {"REST": "User", "SQL": "Database", "Events": "Message Bus"}
