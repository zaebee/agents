...
class ChemicalReaction:
    ...
    def __str__(self) -> str:
        ...
        catalyst_str = f" --{', '.join(self.catalysts)}--> " if self.catalysts else " → "
...
class ReactionEngine:
    ...
    def _initialize_standard_reactions(self):
        ...
        self.register_reaction(ChemicalReaction(
        ...
        self.register_reaction(ChemicalReaction(
            ...
            heat_released=30.0,
            conditions={"pattern": "G→C→A→G", "purpose": "listen_and_react", "event_bus": "available"}
        ))
        ...
        self.register_reaction(ChemicalReaction(
            ...
            heat_released=45.0,
            conditions={"pattern": "G→C→A→C", "purpose": "heal_mutations", "immunity": "active"}
        ))
        ...
        self.register_reaction(ChemicalReaction(
            ...
            heat_released=75.0,
            conditions={"pattern": "multi_step", "purpose": "orchestrate_becoming", "complexity": "high"}
        ))
    ...
    def find_possible_reactions(self, available_reactants: List[str], 
                               available_catalysts: List[str] = None) -> List[ChemicalReaction]:
        """Find all possible reactions given available reactants"""
        possible = []
        
        for reaction in self._known_reactions.values():
            if self._can_react(reaction, available_reactants, available_catalysts or []):
                possible.append(reaction)
        
        return possible
    
    def _can_react(self, reaction: ChemicalReaction, 
                   available_reactants: List[str],
                   available_catalysts: List[str]) -> bool:
        ...
        for catalyst in reaction.catalysts:
            if catalyst not in available_catalysts and catalyst not in self._available_catalysts:
                return False
        
        return True
    
    def execute_reaction(self, reaction: ChemicalReaction,
                        available_reactants: List[str],
                        available_catalysts: List[str] = None,
                        temperature: float = 298.0,
                        pressure: float = 1.0) -> ReactionResult:
        ...
        catalyst_efficiency = self._calculate_catalyst_efficiency(reaction.catalysts, available_catalysts)
        ...
        reaction_time = self._calculate_reaction_time(reaction, catalyst_efficiency, temperature)
        ...
        print(f"✅ Reaction completed successfully!")
    ...
    def _calculate_catalyst_efficiency(self, required_catalysts: List[str],
                                     available_catalysts: List[str]) -> float:
        ...
        for catalyst in required_catalysts:
            if catalyst in available_catalysts and catalyst in self._available_catalysts:
                efficiency = self._available_catalysts[catalyst]["efficiency"]
                total_efficiency *= efficiency
        
        return total_efficiency
    
    def _check_reaction_conditions(self, reaction: ChemicalReaction,
                                 temperature: float, pressure: float) -> bool:
    ...
    def _calculate_reaction_time(self, reaction: ChemicalReaction,
    ...
    def _generate_byproducts(self, reaction: ChemicalReaction,
                           catalyst_efficiency: float) -> List[str]:
    ...
    def find_reactions_by_codon(self, sacred_codon: SacredCodon) -> List[ChemicalReaction]:
        """Find all reactions that implement a specific Sacred Codon"""
        return [reaction for reaction in self._known_reactions.values() 
    ...
    def validate_codon_pattern(self, reaction: ChemicalReaction) -> bool:
        ...
        if reaction.sacred_codon == SacredCodon.HANDLE_COMMAND:
            ...
            return len(element_types) >= 3  # Should use at least 3 different ATCG elements
    ...
    def get_codon_statistics(self) -> Dict[str, int]:
        ...
        unmapped = len([r for r in self._known_reactions.values() if r.sacred_codon is None])
    ...
    def get_reaction_statistics(self) -> Dict[str, float]:
        ...
        successful_reactions = sum(1 for _, result in self._reaction_history if result.success)
        
        total_energy_consumed = sum(result.energy_consumed for _, result in self._reaction_history)
        total_energy_released = sum(result.energy_released for _, result in self._reaction_history)
        
        avg_reaction_time = sum(result.reaction_time for _, result in self._reaction_history) / total_reactions
