...
class ATCGFormatter:
    ...
    def _classify_import(self, import_line: str) -> int:
        ...
        if any(
            word in import_line_lower
            for word in ["aggregate", "entity", "domain", "model"]
        ):
        ...
        # T - Transformations, services, processors
            ...
            word in import_line_lower
            for word in ["service", "transform", "process", "calculat"]
        ):
        ...
        # C - Connectors, adapters, repositories
            ...
            word in import_line_lower
            for word in ["adapter", "connector", "repository", "controller", "client"]
        ):
    ...
    def _group_imports_by_category(
        self, sorted_imports: List[Tuple[int, str, int]]
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if element_type and self._needs_docstring(node):
                    ...
                    if len(lines) > insert_line and lines[
                        insert_line
                    ].strip().startswith('"""'):
    ...
    def _classify_class(self, class_name: str) -> Optional[str]:
        ...
        if any(
            suffix.lower() in class_name_lower
            for suffix in self.naming_patterns["aggregate_suffixes"]
        ):
            return "aggregate"
        elif any(
            suffix.lower() in class_name_lower
            for suffix in self.naming_patterns["transformation_suffixes"]
        ):
            return "transformation"
        elif any(
            suffix.lower() in class_name_lower
            for suffix in self.naming_patterns["connector_suffixes"]
        ):
            return "connector"
        elif any(
            suffix.lower() in class_name_lower
            for suffix in self.naming_patterns["event_suffixes"]
        ):
            return "event"
        ...
        if element_type == "aggregate":
            ...
            if not any(
                ...
                if "calculat" in class_name_lower:
                    return (
                        f"{class_name}Calculator"
                        if not class_name.endswith("Calculator")
                        else class_name
                    )
                else:
    ...
    def _suggest_method_name(self, method_name: str) -> str:
        ...
        if "command" in method_lower and not method_lower.startswith("handle_"):
            return (
                ...
                if not method_name.startswith("handle_")
                else method_name
            )

        # Query pattern
        if any(
            word in method_lower for word in ["get", "find", "retrieve"]
        ) and not method_lower.startswith("query_"):
        ...
        if "event" in method_lower and not (
            method_lower.startswith("on_") or method_lower.startswith("handle_")
        ):
            return (
                f"on_{method_name}"
    ...
    def _classify_file_by_content(self, content: str) -> Optional[str]:
        ...
        if any(
            ...
            for word in ["aggregate", "entity", "class.*aggregate"]
        ):
            return "aggregate"
        elif any(
            word in content_lower
            for word in ["service", "transform", "calculat", "process"]
        ):
            return "transformation"
        elif any(
            word in content_lower
            for word in ["adapter", "connector", "repository", "controller"]
        ):
            return "connector"
        elif any(
            word in content_lower
            for word in ["event", "genesis", "happened", "occurred"]
        ):
            return "event"
...
"""'''
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        for rule in FormatRule:
            rule_counts[rule.value] = len(
                [c for c in self.changes_applied if c.rule == rule]
            )
        ...
        return {
            "summary": {
                ...
                "most_common_rule": max(rule_counts.items(), key=lambda x: x[1])[0]
                if rule_counts
                else None,
            },
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        for rule, count in report["rule_counts"].items():
            if count > 0:
                ...
                print(
                    f"  {emoji.get(rule, '•')} {rule.replace('_', ' ').title()}: {count}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ATCG Architectural Formatter - Auto-format to Sacred Patterns"
    )
